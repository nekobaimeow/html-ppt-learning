...
### Phase 0: 联网搜集知识 + 图片脉络规划（新增 · 必须先执行）

**在进入知识拆解前，必须完成以下两步。**

#### Step 0.1: 联网搜集主题知识

收到用户主题后，**立即启动多维度联网搜索**（使用 `delegate_task` 并行搜索 + 必要时查文献）：

| 搜索维度 | 内容 | 方法 |
|---------|------|------|
| 核心概念 | 主题的定义、原理、关键术语 | web search / Wikipedia |
| 深度知识 | 公式、数据、研究成果、历史演进 | 学术搜索 / arXiv / 文献 |
| 实践应用 | 产品、案例、操作流程、对比 | 产品官网 / 评测 / 论坛 |
| 文化/艺术关联 | 如主题涉及视觉、艺术、文化层面 | 博物馆 / 艺术史 / 设计网站 |
| 最新进展 | 近 1-2 年的新技术/新发现 | 新闻 / 技术博客 / GitHub |

**输出：** 结构化知识库文件（Markdown，分章节，含数据表格和公式），作为后续所有 Phase 的知识输入。

#### Step 0.2: 产出「图片脉络表」

基于搜集的知识，**在写任何一行 HTML 之前**，先产出完整的图片规划表。这是 Phase 3（批量生图）的输入。
...
| 编号 | 文件名 | 用途 (Slide) | 图片类型 | 风格 | 内容描述 / 生图 Prompt | 来源 |
|------|--------|-------------|---------|------|----------------------|------|
| 01 | cover_hero.jpg | S1 封面 | 摄影/艺术照 | 暖色调 · 主题氛围 | A warm photorealistic scene of... | AI: MiniMax |
| 02 | arch_diagram.jpg | S3 架构 | 手绘架构图 | 白板风格 · 手写标注 | Hand-drawn diagram showing... | AI: MiniMax |
| 03 | iphone_product.jpg | S5 设备展示 | 产品摄影 | 白底 · 专业布光 | — | DL: apple.com |
| ... | ... | ... | ... | ... | ... | ... |
```

**图片类型（5 种，按场景选择）：**

| 类型 | 何时用 | Prompt 关键词 | 示例场景 |
|------|--------|-------------|---------|
| 🖐️ **手绘/白板架构图** | 系统架构、流程、概念关系 | `hand-drawn diagram, whiteboard style, marker annotations, clean` | 传感器原理图、数据流图 |
| 📷 **摄影/照片** | 实物产品、自然场景、人物 | `photorealistic, product photography` | 设备展示、自然风光 |
| 🎨 **插画/图示** | 抽象概念、数据可视化、过渡页 | `infographic, illustration, clean modern` | 指标对比、阶段过渡 |
| 🖼️ **艺术照片** | 封面、主题氛围、情感表达 | `artistic, golden hour, moody lighting` | 封面图、章节分隔 |
| 🔬 **科研/医学插图** | 解剖、生物、化学结构 | `scientific illustration, labeled diagram` | 人体结构、分子模型 |

**图片来源决策树：**
```
需要图片？
  ├─ 是具体的知名产品/地标/名画？
  │   └─ ✅ 直接下载官方图片（curl/wget）→ 标注 "DL: <URL>"
  ├─ 是通用概念/抽象原理/示意图？
  │   └─ ✅ AI 生成（MiniMax image-01）→ 写英文 prompt → 标注 "AI: MiniMax"
  └─ 风格要求特殊（手绘/油画/水墨）？
      └─ ✅ AI 生成 + 在 prompt 中强调风格
```

**铁律：**
- **每张图必须有明确的 slide 归属**（编号对应页码）
- **prompt 必须用英文**（MiniMax API 要求），20-200 字符
- **产品图优先下载**（官网 > 评测站 > Wikimedia），下载失败再补 AI 生成
- **架构图优先手绘风格**（`hand-drawn, whiteboard style`），更亲切、不枯燥
- 图片数量 ≥ 幻灯片数 × 0.8（即 30 页课件至少 24 张图）
